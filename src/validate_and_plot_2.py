...
def main(ts1: str, ts2: str, model_path=MODEL_PATH):
    ...
    with (catch_warnings(action='ignore')):
        ...
        source_df = source_df[source_df['patient_id'] == '13']
        val_df = get_training_dataset(ts2)
        val_df = val_df[val_df['patient_id'] == '13']
        val_df.drop(columns=['time', 'event_length', 'patient_id'], inplace=True)

    print("testing")

    # Always run generation for now
    # if not os.path.exists('./generated.csvgenerated_13_scaled.csv'):
    ...
    gen_df = pd.read_csv('./generated.csvgenerated_13_scaled.csv')
