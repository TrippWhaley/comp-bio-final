...
class GlobalsVars():
    def __init__(self, iter_id=0, source='24H', target='30D'):
        ...
        if self.iter_id < 10:
            self.filename = os.path.join(self.current_file_path,
                                         '/Users/trippwhaley/Projects/comp-bio-final/src/Func_Pheno_45k_scaled_with_{1:}_{2:}_tps_source_{3:}_target_{4:}_matched.csv'.format(
                                             self.iter_id, self.source, self.target, self.source, self.target))
        ...
        self.dir_path_main = os.path.join(self.current_file_path,
                                          '../EnGen_train_iterations/engen_output/' + self.folder_name)
        ...
        self.df_source = self.df_gt.iloc[:, :47]
        self.df_target = self.df_gt.iloc[:, 47:]
...
class cytofDataset(Dataset):
    ...
    def __getitem__(self, idx):
        one_row_source = self.cytof_df.iloc[idx, :47].values.astype(float)
        one_row_target = self.cytof_df.iloc[idx, 47:].values.astype(float)
